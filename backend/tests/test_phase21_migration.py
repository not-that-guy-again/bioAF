...
async def test_platform_config_pubsub_keys_exist(session):
    ...
    result = await session.execute(
        text("SELECT key, value FROM platform_config WHERE key = ANY(:keys)").bindparams(keys=list(expected.keys()))
    )
    ...
    for key, default_value in expected.items():
        ...
        assert rows[key] == default_value, f"platform_config['{key}'] expected '{default_value}', got '{rows[key]}'"
...
async def test_platform_config_pubsub_keys_idempotent(session):
    ...
    result = await session.execute(text("SELECT count(*) FROM platform_config WHERE key = 'auto_ingest_enabled'"))
