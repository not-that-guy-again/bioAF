...
describe("InfraComponentsPage", () => {
  ...
  // Test 35: Deploy button starts background deploy and shows modal
  it("deploy button starts background deploy and opens progress modal", async () => {
    mockApiGet.mockImplementation((url: string) => {
      if (url.includes("terraform/status")) return Promise.resolve(mockTfStatus());
      if (url.includes("terraform/runs")) return Promise.resolve({ runs: [] });
      if (url.includes("stack/status")) return Promise.resolve(mockStackStatus());
    ...
    // Background deploy endpoint should be called
    expect(mockApiPost).toHaveBeenCalledWith(
  ...
  // Test 36: Modal can be minimized without stopping deploy
  it("minimize button closes modal without cancelling deploy", async () => {
    mockApiGet.mockImplementation((url: string) => {
      if (url.includes("terraform/status")) return Promise.resolve(mockTfStatus());
      if (url.includes("terraform/runs")) return Promise.resolve({ runs: [] });
      if (url.includes("stack/status")) return Promise.resolve(mockStackStatus());
    ...
    await waitFor(() => {
      expect(screen.queryByText(/Deploy Compute Stack/i)).not.toBeInTheDocument();
    });

    // Inline progress panel should still show
    expect(screen.getByText(/Deploying infrastructure/i)).toBeInTheDocument();
  });

  // Test 37: Deploy button becomes progress indicator during active deploy
  it("shows inline progress panel instead of Deploy button during active deploy", async () => {
    mockApiGet.mockImplementation((url: string) => {
      if (url.includes("terraform/status"))
        return Promise.resolve(mockTfStatus({ active_run_id: 42, active_run_status: "applying" }));
      if (url.includes("terraform/runs")) return Promise.resolve({ runs: [] });
      if (url.includes("stack/status")) return Promise.resolve(mockStackStatus());
      if (url.includes("stack/components"))
        return Promise.resolve({ compute_stack: null, compute_deployed: false, storage_deployed: false, components: [] });
    ...
    // View progress button should be available in the inline panel
    expect(screen.getByRole("button", { name: /View progress/i })).toBeInTheDocument();
  });

  // Test 38: View progress re-opens the modal
  it("view progress link re-opens the deploy modal", async () => {
    mockApiGet.mockImplementation((url: string) => {
      if (url.includes("terraform/status"))
        return Promise.resolve(mockTfStatus({ active_run_id: 42, active_run_status: "applying" }));
      if (url.includes("terraform/runs/42"))
        return Promise.resolve({ id: 42, status: "applying", resources_planned: 5, resources_completed: 2, apply_log: null });
      if (url.includes("terraform/runs")) return Promise.resolve({ runs: [] });
      if (url.includes("stack/status")) return Promise.resolve(mockStackStatus());
      if (url.includes("stack/components"))
        return Promise.resolve({ compute_stack: null, compute_deployed: false, storage_deployed: false, components: [] });
