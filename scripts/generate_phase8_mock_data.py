...
async def create_experiments(
    ...
    configs = [
        {
            "name": "Phase8 Demo: Human PBMC scRNA-seq",
            "hypothesis": "PBMC cell type proportions vary with treatment",
        },
        {
            "name": "Phase8 Demo: Mouse Brain Atlas",
            "hypothesis": "Novel cell types in developing mouse cortex",
        },
        {
            "name": "Phase8 Demo: Human Tumor Microenvironment",
    ...
    instrument_configs = [
        {
            ...
            "sequencer_run_id": "230915_A00123_0456_BHKWVTDRX3",
        },
        {
            "instrument_model": "Illumina NextSeq 2000",
            "sequencer_run_id": "231001_VH00234_0078_AACFHGTM5",
        },
        {
            "instrument_model": "Illumina NovaSeq 6000",
            "sequencer_run_id": "231015_A00456_0789_BHJKLNDRX3",
        },
        {
            "instrument_model": "Illumina NextSeq 2000",
            "sequencer_run_id": "231022_VH00567_0123_AACMNOPQ5",
        },
    ]
...
async def create_reviews(
    ...
    for cfg in review_configs:
        ...
        if "flag_sample_idx" in cfg and cfg["flag_sample_idx"] < len(exp_samples):
            ...
            sample_verdicts = [
                {
                    "sample_id": flagged.id,
                    "verdict": "approved_with_caveats",
                    "notes": "Elevated mito reads",
                }
            ]
...
async def main():
    ...
    async with async_session_factory() as session:
        ...
        runs = await create_pipeline_runs(
            session, experiments, samples, org_id, user_id
        )
        ...
        print("  Reviews: 4 (runs 3, 4 are unreviewed)")
