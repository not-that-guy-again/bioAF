...
if [ -n "$VM_IP" ]; then
    ...
    echo "     Log out and back in so Docker permissions take effect, then"
    echo "     clone bioAF and run setup:"
    echo ""
    green "     git clone https://github.com/not-that-guy-again/bioAF.git"
    green "     cd bioAF"
    green "     ./bioaf setup"
    echo ""
    dim "     If you see a Docker permission error, log out (exit) and SSH back in."
    echo ""
